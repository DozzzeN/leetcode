...
//rabbbit
//^^^ ^^^
public class distinct_subsequences02 {
    public static void main(String[] args) {
        System.out.println(new distinct_subsequences02.Solution().numDistinct(
                "babgbag", "bag"
        ));
    }
    ...
    static class Solution {
        ...
        public int numDistinct(String s, int sStart, String t, int tStart) {
            //t串已经到达末尾，选法是1种
            if (tStart == t.length()) {
                memo[sStart][tStart] = 1;
                return 1;
            }
            //s串已经到达末尾，选法是0种
            if (sStart == s.length()) {
                memo[sStart][tStart] = 0;
                return 0;
            }
            if (memo[sStart][tStart] != 0) {
